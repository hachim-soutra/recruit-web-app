...
@if( !empty($data['jobid']) && $data['jobid'] > 0 && !empty($data['firstJobDetail']))
	...
	$meta_canonical = trim(route('common.job-listing').'/'.$job_slug);
	?>
    ...
    <div class="container">
        <div class="bd-block">
            <div class="title-block mb-4">
                <h1>All Jobs in <span style="text-transform:capitalize;">{{ @$data['term'] }}</span></h1>
            </div>
            @if(session('success'))
                <script>
...
<script>
    ...
    function showJobPost(jobpostid, jobtitleslug = null){
		...
		var href="{{ route('job-category') }}" + '/' + "{{ @$data['term'] }}" + '/job/' + job_permalink;
