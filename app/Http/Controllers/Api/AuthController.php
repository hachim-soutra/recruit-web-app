...
class AuthController extends Controller
{
        ...
        if ($validator->fails()) {
            ...
            if (Auth::attempt($credentials)) {
                if ($request->input('fpm_token')) {
                    $user_fpm_token = $request->input('fpm_token');
                ...
                ->update(['fpm_token'=>$user_fpm_token]);
                ...
                if (Auth::user()->status == 0) {
                    return response()->json([
                        "status"     => false,
                        "message"    => "You are not verified; please contact your info@recruit.ie.",
                        "error_type" => 2,
                    ], 200);
                }
                if (Auth::user()->email_verified == 1 && Auth::user()->status == 1) {
                    ...
                    if ($user->user_type != $request->input('login_type')) {
                        $text = $request->input('login_type') == 'employer' ? 'an ' : 'a ';
                        return response()->json([
                            "status"     => false,
                            "message"    => "Please insert a valid credential. You are not ". $text . $request->input('login_type')."For support contact info@recruit.ie",
                            "error_type" => 2,
                        ], 200);
                ...
                return response()->json([
                    "status"     => false,
                    "message"    => "Please insert a valid credential. For support contact info@recruit.ie",
                    "error_type" => 2,
                ], 200);
            }
        }
    ...
    public function register(Request $request)
    {
        ...
        if ($user) {
            ...
            return response()->json([
                ...
                "message"   => "Registration was completed successfully. Please check your registered email for email verification.",
            ], 200);
        } else {
            return response()->json([
                "status"     => false,
    ...
    public function socialMediaLinkedinLogin(Request $request)
    {
        ...
        if ($userdata) {
            ...
            if ($userdata) {
                $text = ((@$userdata->user_type == 'employer') || (@$userdata->user_type != ''))  ? 'an ' : 'a ';
                return response()->json([
                    "status"     => false,
                    "message"    => "Registration Failed. You already registered as ". $text . $userdata->user_type . ".",
                    "error_type" => 2,
                ], 200);
            }
